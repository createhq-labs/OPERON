-- Documents: visibility-aware read access.
-- Supports global, department, private scopes plus explicit allow-lists
-- for user types, roles, teams, and individual user assignments.
    ...
    and exists (
      ...
      where u.auth_user_id = auth.uid()
        and (
          ...
          or (documents.visibility_scope = 'private' and documents.author_legacy_id = u.legacy_id)
          or documents.allowed_user_types  && array[u.user_type]
          or documents.allowed_role_ids    && array[u.role_legacy_id]
          or documents.allowed_team_ids    && array[u.team_legacy_id]
...
create policy "insert documents by authenticated users" on documents
  for insert with check (
    ...
    and exists (
      select 1 from users u
      where u.auth_user_id = auth.uid()
        and documents.author_legacy_id = u.legacy_id
    )
...
create policy "update documents by owner or admin" on documents
  for update using (
    ...
    and exists (
      ...
      where u.auth_user_id = auth.uid()
        and (
          documents.author_legacy_id = u.legacy_id
