...
function assignSemanticChunkIds(blocks: ParserResult["blocks"]): ParserResult["blocks"] {
  let currentChunk = 0;
...
export function enrichParsedDocument(normalized: { parsed: ParserResult; blocks: ParserResult["blocks"]; toc: ParserResult["toc"]; searchableText: string }, job: IngestionJob): EnrichedDocument {
  ...
  blocks.forEach((block) => {
    ...
    chunk.text += ` ${typeof block.content === "string" ? block.content : JSON.stringify(block.content)}`;
  });
