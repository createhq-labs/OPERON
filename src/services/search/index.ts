...
// ─── Fallback search (plain string matching) ──────────────────────────────────
...
function fallbackResourceSearch(
  user: User,
  resources: ResourceItem[],
  query: string,
  category?: string,
  ...
  const filtered         = visibleResources.filter((resource) => {
    ...
    if (!cleanQuery) return true;
    const haystack = [resource.title, resource.description, resource.category, resource.href]
      .join(" ").toLowerCase();
    return haystack.includes(cleanQuery);
  });
...
function sortDocuments<T extends { pinned?: boolean; updatedAt?: string }>(
  items: T[],
  sort: SearchOptions["sort"] = "pinned"
) {
  return items.slice().sort((left, right) => {
    ...
    return (
      ...
      Number(new Date(right.updatedAt || 0)) - Number(new Date(left.updatedAt || 0))
    );
  });
}