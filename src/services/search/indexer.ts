...
export function buildDriveDocumentIndex(documents: DriveDocumentReference[]): SearchEntry<DriveDocumentReference>[] {
  return documents.map((document) => ({
    id: document.id,
    entityType: "drive",
    ref: document,
    ...
    content: joinSearchText(
      ...
      ...(document.permissionSummary ?? []).map((permission) => `${permission.role} ${permission.emailAddress ?? ""}`),
    ),
    tags: [document.tag],
    departmentId: document.departmentId,
    updatedAt: document.updatedAt,
...
export function buildResourceIndex(resources: ResourceItem[]): SearchEntry<ResourceItem>[] {
  return resources.map((resource) => ({
    ...
    departmentId: (resource as any).departmentId ?? resource.allowedDepartments?.[0],
...
export async function indexDocument(entry: SearchEntry<any>) {
  console.debug("search.indexDocument", { id: entry.id, entityType: entry.entityType, tags: entry.tags });
}
